...
//            initRead();
        ...
        FLASHMEM int DiffBuffDummyRot::readDiff(int& x, int& y, int& len, int scanline)
            {
            const int MAX_WRITE_LINE_ROT = 80;
            if (_current_line >= _end) return -1; // we are done. 
            if (scanline >= _end)
                { // scanline after end of drawing, go as fast as possible. 
                x = 0;
                y = _current_line;
                if (_current_line + MAX_WRITE_LINE_ROT <= _end)
                    {
                    len = MAX_WRITE_LINE_ROT * DiffBuffBase::LY;
                    _current_line += MAX_WRITE_LINE_ROT;
                    }
                else
                    {
                    len = (_end - _current_line) * DiffBuffBase::LY;
                    _current_line = _end;
                    }
                return 0;
                }
            ...
            if (maxl > MAX_WRITE_LINE_ROT) maxl = MAX_WRITE_LINE_ROT; // not too much lines at once
            len = maxl * DiffBuffBase::LY;
            _current_line += maxl;
            return 0;
            }


        FLASHMEM int DiffBuffDummyRot::scanlineStartInit()
            {
            initRead();
            if (_current_line >= _end) return -1; // we are done. 
            //if (scanline >= _end)
