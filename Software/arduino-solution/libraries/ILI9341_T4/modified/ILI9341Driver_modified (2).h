...
namespace ILI9341_T4
{
    ...
    *******************************************************************************************/
    ...
    class DiffBuffDummyRot : public DiffBuffBase
        {
        ...
        public:
            ...
            /** ctor */
            DiffBuffDummyRot() : DiffBuffBase(), _current_line(0), _begin(0), _end(DiffBuffBase::LX), _rawnb(0)
                {
                initRead();
                initRaw();
                }
            ...
            /** create a diff that redraws every line in [begin, end[ */
            void computeDummyDiff(int begin = 0, int end = DiffBuffBase::LX)
                {
                _begin = (begin < 0) ? 0 : begin;
                _end = (end > DiffBuffBase::LX) ? DiffBuffBase::LX : end;
                initRead();
                }
            ...
            virtual void readRaw(int& nbwrite, int& nbskip) override
                ...
                if (_rawnb == 0)
                    ...
                    _rawnb = 1;
                    nbwrite = 0;
                    nbskip = DiffBuffBase::LY * _begin;
                    if (nbskip > 0) return;
                    }
                if (_rawnb == 1)
                    {
                    _rawnb = 2;
                    nbwrite = DiffBuffBase::LY * (_end - _begin);
                    nbskip = DiffBuffBase::LY * (DiffBuffBase::LX - _end);
                    }
                nbwrite = 0;
                nbskip = DiffBuffBase::LY * DiffBuffBase::LX + 1;
                }
