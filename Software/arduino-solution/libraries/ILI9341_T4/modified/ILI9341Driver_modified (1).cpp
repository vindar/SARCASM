...
#define ILI9341_T4_PWCTR6  0xFC
    ...
    ***********************************************************************************************************/
    ...
    FLASHMEM bool ILI9341Driver::begin(uint32_t spi_clock, uint32_t spi_clock_read)
        ...
        while(1)
            ...
            delay(60); // must wait for the screen to exit sleep mode. 
            _beginSPITransaction(_spi_clock / 4); // quarter speed for setup ! 
    ...
    ***********************************************************************************************************/
    ...
    FLASHMEM void ILI9341Driver::_setRealOrientation(uint8_t m)
        {
        m = _clip(m, (uint8_t)0, (uint8_t)3);        
        _waitUpdateAsyncComplete();
        _mirrorfb = nullptr; // force full redraw.
        ...
        switch (m) 
            {
            case 0:
            _writedata8_last(ILI9341_T4_MADCTL_MX | ILI9341_T4_MADCTL_BGR);
            break;
        case 1:
            _writedata8_last(ILI9341_T4_MADCTL_MV | ILI9341_T4_MADCTL_BGR);
            break;
        case 2:
            _writedata8_last(ILI9341_T4_MADCTL_MY | ILI9341_T4_MADCTL_BGR);
            break;
        case 3:
            _writedata8_last(ILI9341_T4_MADCTL_MX | ILI9341_T4_MADCTL_MY | ILI9341_T4_MADCTL_MV | ILI9341_T4_MADCTL_BGR);
            break;
            }        
        
        //writecommand_last(ILI9341_T4_NOP);
        _endSPITransaction();
        resync();
        }
    ...
    ***********************************************************************************************************/
    ...
    FLASHMEM void ILI9341Driver::updateRegion(bool redrawNow, const uint16_t* fb, int xmin, int xmax, int ymin, int ymax, int stride)
        {
        _TLX = ILI9341_T4_TFTWIDTH;
        _TLY = ILI9341_T4_TFTHEIGHT;
    ...
    FLASHMEM void ILI9341Driver::update(const uint16_t* fb, bool force_full_redraw)
        ...
        if (fb == _fb1)
            ...
            _TLY = ILI9341_T4_TFTWIDTH;
            _TLX = ILI9341_T4_TFTHEIGHT;


            //_dummydiff1->computeDiff(_fb1, fb, getRotation(), _diff_gap, true, _compare_mask);
            _ddrot.computeDummyDiff(); 

            _flush_cache(_fb1, ILI9341_T4_NB_PIXELS * 2);
            _updateAsync(_fb1, &_ddrot); // launch update
    ...
    FLASHMEM void ILI9341Driver::_updateAsync(const uint16_t* fb, DiffBuffBase* diff)
        ...
        */
        ...
        _writecommand_cont(ILI9341_T4_CASET); 
        _writedata16_cont(x);
        _writedata16_cont(_TLX);
        _writecommand_cont(ILI9341_T4_PASET);
        _writedata16_cont(y);
        _writedata16_last(_TLY);
        _endSPITransaction();
        _prev_caset_x = x;
        _prev_paset_y = y;
    ...
    FLASHMEM void ILI9341Driver::_subFrameTimerStartcb2()
        ...
        int x = 0, y = 0, len = 0;
        int asl = 2 * _TLY; // make sure readDiff returns 0.  (_vsync_spacing > 0) ? _slinitpos  : (2 * _TLY);
        ...
        _last_y = (_TLX * y + x + len) / _TLX;
        _stats_nb_uploaded_pixels = len;

        _dmatx.sourceBuffer(_fb + x + (y * _TLX), 2 * len);
    ...
    FLASHMEM void ILI9341Driver::_subFrameInterruptDiff()
        ...
        if (_vsync_spacing > 0)
            ...
            int m = _last_y + _TLY - _slinitpos - _nbScanlineDuring(_em_async);
            if (m < _margin) _margin = m;
            }
        int x = 0, y = 0, len = 0;
        int asl = (_vsync_spacing > 0) ? (_slinitpos + _nbScanlineDuring(_em_async)) : (2 * _TLY);
        int r = _diff->readDiff(x, y, len, asl);

        //Serial.printf("sub: x= %d y=%d len=%d r=%d asl=%d\n", x, y, len, r, asl);  // debugging
        ...
        if (_hardware_dc)
            ...
            _dmatx.sourceBuffer(_fb + x + (y * _TLX), len * 2);
            _last_y = (_TLX * y + x + len) / _TLX;
            _stats_nb_uploaded_pixels += len;
